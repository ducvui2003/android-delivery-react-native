...
export const isLogin = async (): Promise<void> => {
	const accessToken = await
		getFromStorage(KEY_SECURE.ACCESS_TOKEN);
	if (accessToken !== null)
		return;
...
export const loginApi = async (data: LoginFormType): Promise<User> => {
	try {
		const result = await
			axiosInstance.post<ApiResponse<ResponseAuthentication>>(EndPoint.LOGIN, data);
		await setToStorage(KEY_SECURE.ACCESS_TOKEN, result.data.data.access_token);
...
export const registerApi = async (data: RegisterFormType): Promise<void> => {
	try {
		const result = await
			axiosInstance.post(EndPoint.REGISTER, data);
	} catch (error: unknown) {
		if (axios.isAxiosError(error)) {
			const errorMessage = error.response?.data?.message || "An error occurred";
