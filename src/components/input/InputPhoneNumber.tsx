...
function InputPhoneNumber({
  ...
  const [countryPhoneNumber, setCountryPhoneNumber] =
    useState<CountryPhoneNumberType>(countries[0]);
    ...
    if (phoneNumber.length <= 9 || phoneNumber.length >= 15) {
      ...
      onValidation(
        phoneUtil.isValidNumber(
          phoneUtil.parse(phoneNumber, countryPhoneNumber.code)
        )
      );
    }
  };
    ...
    <InputIcon
      ...
      icon={
        <Row style={[styles.container]}>
          ...
          />
          <Text
            style={[styles.textDialCode, { color: theme.dialCode.getColor() }]}
          >
